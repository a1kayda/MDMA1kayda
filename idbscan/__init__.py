...
def distance(element_1, element_2):
    ...
    """
    euclidean_distance = ((element_1['1_feature'] - element_2['1_feature']) ** 2 +
                          (element_1['2_feature'] - element_2['2_feature']) ** 2 +
                          (element_1['3_feature'] - element_2['3_feature']) ** 2) ** (1 / 2)
    return euclidean_distance.iloc[0].astype(float)


class IncrementalDBSCAN:

    def __init__(self, eps=0.103, min_samples=10):
        """
        ...
        """
        self.dataset = pd.DataFrame(columns=['1_feature', '2_feature', '3_feature'], dtype=np.float)
        self.labels = pd.DataFrame(columns=['Label'])
        self.final_dataset = pd.concat([pd.DataFrame(columns=['1_feature', '2_feature', '3_feature'], dtype=np.float), pd.DataFrame(columns=['Label'])], ignore_index=True)
        self.mean_core_elements = pd.concat([pd.DataFrame(columns=['1_feature', '2_feature', '3_feature'], dtype=np.float), pd.DataFrame(columns=['Label'])], ignore_index=True)
        self.eps = eps
    ...
    def set_data(self, message):
        ...
        temp.columns = ['1_feature', '2_feature', '3_feature']
    ...
    def add_labels_to_dataset(self, labels):
        ...
        self.final_dataset = pd.concat([self.dataset, self.labels], axis=1, ignore_index=True)
    ...
    def find_mean_core_element(self):
        ...
        self.mean_core_elements = self.mean_core_elements \
            .groupby('Label')['1_feature', '2_feature', '3_feature'].mean()
    ...
    def check_min_samples_in_eps_or_outlier(self, min_dist_index):
        ...
        for index, cluster_element in nearest_cluster_elements.iterrows():
            if (cluster_element['1_feature'] - self.eps
                <= float(new_element['1_feature']) <= cluster_element['1_feature'] + self.eps) \
                    and (cluster_element['2_feature'] - self.eps
                         <= float(new_element['2_feature']) <= cluster_element['2_feature'] + self.eps) \
                    and (cluster_element['3_feature'] - self.eps
                         <= float(new_element['3_feature']) <= cluster_element['3_feature'] + self.eps):
                min_samples_count += 1
        ...
        if min_samples_count >= self.min_samples:
            ...
            new_cluster_elements = pd.DataFrame(columns=['Index'], dtype=np.float)
            for index, outlier in outliers.iterrows():
                if (outlier['1_feature'] - self.eps
                    <= float(new_element['1_feature']) <= outlier['1_feature'] + self.eps) \
                        and (outlier['2_feature'] - self.eps
                             <= float(new_element['2_feature']) <= outlier['2_feature'] + self.eps) \
                        and (outlier['3_feature'] - self.eps
                             <= float(new_element['3_feature']) <= outlier['3_feature'] + self.eps):
                    min_outliers_count += 1
                    new_cluster_elements = pd.concat([pd.DataFrame({"Index": index}), new_cluster_elements], ignore_index=True)
    ...
    def incremental_dbscan_(self):
        self.final_dataset = self.final_dataset.append({'1_feature': self.dataset.iloc[-1]['1_feature'],
                                                        '2_feature': self.dataset.iloc[-1]['2_feature'],
                                                        '3_feature': self.dataset.iloc[-1]['3_feature'],
    ...
    def find_largest_cluster(self):
        ...
        # cluster_size = cluster_size['1_feature'].value_counts()
        try:
            print(cluster_size)
